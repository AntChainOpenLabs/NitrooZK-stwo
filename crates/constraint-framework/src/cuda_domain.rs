...
pub struct CudaDomainEvaluator<'a> {
    pub trace_eval: &'a TreeVec<Vec<&'a CircleEvaluation<CudaBackend, BaseField, BitReversedOrder>>>,
...
impl<'a> CudaDomainEvaluator<'a> {
    #[allow(dead_code)]
    pub fn new(
        trace_eval: &'a TreeVec<Vec<&CircleEvaluation<CudaBackend, BaseField, BitReversedOrder>>>,
        row: usize,
...
impl EvalAtRow for CudaDomainEvaluator<'_> {
    type F = BaseField;
    type EF = SecureField;
