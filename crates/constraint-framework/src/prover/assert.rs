...
pub fn assert_constraints_on_polys_cuda<E: crate::FrameworkEval + Sync>(
    trace_polys: &TreeVec<Vec<CirclePoly<stwo::prover::backend::cuda::CudaBackend>>>,
    trace_domain: CanonicCoset,
    eval_log_size: u32,
    evaluator: &E,
    n_constraints: usize,
    ...
    let cpu_trace_polys: TreeVec<Vec<CirclePoly<SimdBackend>>> = trace_polys.as_ref().map(|tree| {
        tree.iter()
            .map(|cuda_poly| {
                // Transfer coefficients from GPU to CPU
                let cpu_coeffs = BaseColumn::from_cpu(cuda_poly.coeffs.to_cpu());
                CirclePoly::new(cpu_coeffs)
            })
            .collect_vec()
    });
