...
__global__ void evaluate_call_opcode_rel_imm_pre_kernel(
    ...
    evaluate_decode_instruction_2a7a2(
        input_pc_col0,
        decode_output,
        call_opcode_rel_imm_eval->verify_instruction_lookup_elements,
        &cuda_evaluator
    );
    ...
    evaluate_read_positive_num_bits_29(
        ...
        read_fp_output,
        call_opcode_rel_imm_eval->memory_address_to_id_lookup_elements,
        call_opcode_rel_imm_eval->memory_id_to_big_lookup_elements,
        &cuda_evaluator
    );
    ...
    // Add first opcodes relation entry (positive)
    {
        ...
        RelationEntry<3> entry(
            call_opcode_rel_imm_eval->opcode_lookup_elements,
...
void evaluate_call_opcode_rel_imm(
    ...
    if (use_assert_evaluator) {
        evaluate_call_opcode_rel_imm_pre_kernel<CudaAssertEvaluator><<<num_blocks, block_dim>>>(
            numerators,
        ...
        evaluate_call_opcode_rel_imm_pre_kernel<CudaEvaluator><<<num_blocks, block_dim>>>(
            numerators,
            device_trace1_evaluations,
            random_coeff_powers,
            domain_log_size,
