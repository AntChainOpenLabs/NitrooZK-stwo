...
void evaluate_jump_opcode_rel_imm(
    ...
    cuda_mem_copy_host_to_device<JumpOpcodeRelImm_Eval>((JumpOpcodeRelImm_Eval*)eval, device_jump_rel_imm_eval, 1);
    ...
    if (use_assert_evaluator) {
        evaluate_jump_opcode_rel_imm_pre_kernel<CudaAssertEvaluator><<<num_blocks, block_dim>>>(
            numerators,
        ...
        evaluate_jump_opcode_rel_imm_pre_kernel<CudaEvaluator><<<num_blocks, block_dim>>>(
            numerators,
            device_trace1_evaluations,
            random_coeff_powers,
            domain_log_size,
