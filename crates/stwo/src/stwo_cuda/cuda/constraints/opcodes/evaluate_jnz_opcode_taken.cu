...
__global__ void evaluate_jnz_opcode_taken_pre_kernel(
    ...
    evaluate_decode_instruction_de75a(
        ...
        decode_output,
        jnz_opcode_taken_eval->verify_instruction_lookup_elements,
        &cuda_evaluator
    );
    ...
    evaluate_read_positive_num_bits_252<EvaluatorT>(
        ...
        dst_limb_27_col35,
        read_dst_output,
        jnz_opcode_taken_eval->memory_address_to_id_lookup_elements,
        jnz_opcode_taken_eval->memory_id_to_big_lookup_elements,
        &cuda_evaluator
    );
    ...
    // Add first opcodes relation entry (positive)
    {
        ...
        RelationEntry<3> entry(
            jnz_opcode_taken_eval->opcode_lookup_elements,
...
void evaluate_jnz_opcode_taken(
    ...
    if (use_assert_evaluator) {
        evaluate_jnz_opcode_taken_pre_kernel<CudaAssertEvaluator><<<num_blocks, block_dim>>>(
            numerators,
        ...
        evaluate_jnz_opcode_taken_pre_kernel<CudaEvaluator><<<num_blocks, block_dim>>>(
            numerators,
            device_trace1_evaluations,
            random_coeff_powers,
            domain_log_size,
