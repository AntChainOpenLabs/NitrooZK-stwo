...
#define VERIFY_BITWISE_XOR_4_THREAD_COUNT_MAX 256

template <typename EvaluatorT>
__launch_bounds__(256, 2)
__global__ void evaluate_verify_bitwise_xor_4_pre_kernel(
    qm31 *numerators,
    ...
    VerifyBitwiseXor_4_Eval *xor_eval,
    qm31 cumsum_shift,
    ...
    // Layout convention:
    // - First 3 columns are preprocessed columns BitwiseXor::new(4, i).id()
    ...
    RelationEntry<3> entry(
        xor_eval->verify_bitwise_xor_4_lookup_elements,
        multiplicity_ext,
        values
    );
...
extern "C"
void evaluate_verify_bitwise_xor_4(
    ...
    VerifyBitwiseXor_4_Eval *xor_eval =
        (VerifyBitwiseXor_4_Eval *)eval;
    unsigned int eval_domain_size = 1u << eval_domain_log_size;

    // Combine preprocessed columns and multiplicity columns
    ...
    VerifyBitwiseXor_4_Eval *device_xor_eval =
        cuda_malloc<VerifyBitwiseXor_4_Eval>(1);
    cuda_mem_copy_host_to_device<VerifyBitwiseXor_4_Eval>(
    ...
    timer global_timer;
    global_timer.start("evaluate_verify_bitwise_xor_4");

    int block_dim = eval_domain_size < VERIFY_BITWISE_XOR_4_THREAD_COUNT_MAX
        ? eval_domain_size
        : VERIFY_BITWISE_XOR_4_THREAD_COUNT_MAX;
    int num_blocks = (eval_domain_size + block_dim - 1) / block_dim;

    if (use_assert_evaluator) {
        evaluate_verify_bitwise_xor_4_pre_kernel<CudaAssertEvaluator><<<
        ...
        evaluate_verify_bitwise_xor_4_pre_kernel<CudaEvaluator><<<
            num_blocks, block_dim>>>(
            numerators,
            device_trace0_evaluations,
            device_trace1_evaluations,
    ...
    ASSERT_CUDA_SUCCESS(cudaDeviceSynchronize());
    ASSERT_CUDA_SUCCESS(cudaGetLastError());
    global_timer.end("evaluate_verify_bitwise_xor_4");
