...
#define VERIFY_BITWISE_XOR_8_B_THREAD_COUNT_MAX 256

template <typename EvaluatorT>
__launch_bounds__(256, 2)
__global__ void evaluate_verify_bitwise_xor_8_b_pre_kernel(
    qm31 *numerators,
    ...
    VerifyBitwiseXor_8_B_Eval *xor_eval,
    qm31 cumsum_shift,
    ...
    RelationEntry<3> entry(
        xor_eval->verify_bitwise_xor_8_b_lookup_elements,
        multiplicity_ext,
        values
    );
...
extern "C"
void evaluate_verify_bitwise_xor_8_b(
    ...
    VerifyBitwiseXor_8_B_Eval *xor_eval =
        (VerifyBitwiseXor_8_B_Eval *)eval;
    ...
    VerifyBitwiseXor_8_B_Eval *device_xor_eval =
        cuda_malloc<VerifyBitwiseXor_8_B_Eval>(1);
    cuda_mem_copy_host_to_device<VerifyBitwiseXor_8_B_Eval>(
    ...
    timer global_timer;
    global_timer.start("evaluate_verify_bitwise_xor_8_b");

    int block_dim = eval_domain_size < VERIFY_BITWISE_XOR_8_B_THREAD_COUNT_MAX
        ? eval_domain_size
        : VERIFY_BITWISE_XOR_8_B_THREAD_COUNT_MAX;
    int num_blocks = (eval_domain_size + block_dim - 1) / block_dim;

    if (use_assert_evaluator) {
        evaluate_verify_bitwise_xor_8_b_pre_kernel<CudaAssertEvaluator><<<
        ...
        evaluate_verify_bitwise_xor_8_b_pre_kernel<CudaEvaluator><<<
            num_blocks, block_dim>>>(
            numerators,
            device_trace0_evaluations,
            device_trace1_evaluations,
    ...
    ASSERT_CUDA_SUCCESS(cudaDeviceSynchronize());
    ASSERT_CUDA_SUCCESS(cudaGetLastError());
    global_timer.end("evaluate_verify_bitwise_xor_8_b");
