...
#define RANGE_CHECK_18_THREAD_COUNT_MAX 256

template<typename EvaluatorT>
__launch_bounds__(256, 2)
__global__ void evaluate_range_check_18_pre_kernel(
    qm31 *numerators,
...
void evaluate_range_check_18(
    ...
    RangeCheck_18_Eval *range_eval = (RangeCheck_18_Eval *) eval;
    ...
    RangeCheck_18_Eval *device_range_eval =
        cuda_malloc<RangeCheck_18_Eval>(1);
    cuda_mem_copy_host_to_device<RangeCheck_18_Eval>(
    ...
    timer global_timer;
    global_timer.start("evaluate_range_check_18");

    int block_dim = eval_domain_size < RANGE_CHECK_18_THREAD_COUNT_MAX
        ? eval_domain_size
        : RANGE_CHECK_18_THREAD_COUNT_MAX;
    int num_blocks = (eval_domain_size + block_dim - 1) / block_dim;

    if (use_assert_evaluator) {
        evaluate_range_check_18_pre_kernel<CudaAssertEvaluator><<<
        ...
        evaluate_range_check_18_pre_kernel<CudaEvaluator><<<
            num_blocks, block_dim>>>(
            numerators,
            device_trace0_evaluations,
            device_trace1_evaluations,
    ...
    ASSERT_CUDA_SUCCESS(cudaDeviceSynchronize());
    ASSERT_CUDA_SUCCESS(cudaGetLastError());
    global_timer.end("evaluate_range_check_18");
