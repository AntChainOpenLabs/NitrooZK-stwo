...
#define RANGE_CHECK_4_3_THREAD_COUNT_MAX 256

template<typename EvaluatorT>
__launch_bounds__(256, 2)
__global__ void evaluate_range_check_4_3_pre_kernel(
    qm31 *numerators,
    ...
    RangeCheck_4_3_Eval *range_eval,
    qm31 cumsum_shift,
    ...
    // Two RangeCheck preprocessed columns + 1 multiplicity
    ...
    RelationEntry<2> entry(
        range_eval->range_check_4_3_lookup_elements,
        multiplicity_ext,
        values
    );
...
void evaluate_range_check_4_3(
    ...
    RangeCheck_4_3_Eval *range_eval = (RangeCheck_4_3_Eval *) eval;
    ...
    RangeCheck_4_3_Eval *device_range_eval =
        cuda_malloc<RangeCheck_4_3_Eval>(1);
    cuda_mem_copy_host_to_device<RangeCheck_4_3_Eval>(
    ...
    timer global_timer;
    global_timer.start("evaluate_range_check_4_3");

    int block_dim = eval_domain_size < RANGE_CHECK_4_3_THREAD_COUNT_MAX
        ? eval_domain_size
        : RANGE_CHECK_4_3_THREAD_COUNT_MAX;
    int num_blocks = (eval_domain_size + block_dim - 1) / block_dim;

    if (use_assert_evaluator) {
        evaluate_range_check_4_3_pre_kernel<CudaAssertEvaluator><<<
        ...
        evaluate_range_check_4_3_pre_kernel<CudaEvaluator><<<
            num_blocks, block_dim>>>(
            numerators,
            device_trace0_evaluations,
            device_trace1_evaluations,
    ...
    ASSERT_CUDA_SUCCESS(cudaDeviceSynchronize());
    ASSERT_CUDA_SUCCESS(cudaGetLastError());
    global_timer.end("evaluate_range_check_4_3");
