...
__launch_bounds__(256, 2)
__global__ void assert_eq_process_constraint_post_kernel(
    qm31 *numerators,
    Fraction *intermediate_fractions,
    unsigned *constraint_index_array,
...
__global__ void assert_eq_evaluate_constraint_quotients_finalize_kernel(
...
void evaluate_assert_eq_opcode(
    ...
    evaluate_assert_eq_pre_kernel<<<num_blocks, block_dim>>>(
        numerators,
    ...
    printf("last batch: %d\n", last_batch);
    assert_eq_process_constraint_post_kernel<<<num_blocks, block_dim>>>(
        numerators,
        d_intermediate_fractions,
        constrain_index_array,
    ...
    assert_eq_evaluate_constraint_quotients_finalize_kernel<<<num_blocks, block_dim>>>(
        quotients_0,
        quotients_1,
        quotients_2,
        quotients_3,
