...
__global__ void evaluate_add_opcode_pre_kernel(
    ...
    // if (row == 0)
    //     printf("row:%d, line:%d, constraint_index:%d\n", row, __LINE__, cuda_evaluator.constraint_index);
    cuda_evaluator.add_constraint(
        sub(
            mem1_base_col13,
            add(
...
void evaluate_add_opcode(
    ...
    if (use_assert_evaluator) {
        evaluate_add_opcode_pre_kernel<CudaAssertEvaluator><<<num_blocks, block_dim>>>(
            numerators,
        ...
        evaluate_add_opcode_pre_kernel<CudaEvaluator><<<num_blocks, block_dim>>>(
            numerators,
            device_trace1_evaluations,
            random_coeff_powers,
            domain_log_size,
