 ...
 * GPU-resident pedersen table (g_pedersen_table_small_columns). No CPU table needed.
 *
 * Architecture:
 *   - 1 thread per row
 ...
 *   - Uses g_pedersen_table_small_columns from pedersen_table_init.cu
 *
 * Column layout (234 columns):
 ...
 *   65-148:  PEM chain 0 output (28 zeros + 28 result_x + 28 result_y)
 *   149-232: PEM chain 1 output (28 zeros + 28 result_x + 28 result_y)
 *   233:     Multiplicity
 *
...
#include "gen_pedersen_aggregator_wb9_trace.cuh"
#include "../fields.cuh"
...
// ============================================================================
// Pedersen table (small / 9-bit) -- defined in pedersen_table_init.cu
// ============================================================================

#define PEDERSEN_TABLE_SMALL_N_COLUMNS 56
extern __device__ m31* g_pedersen_table_small_columns[56];

// ============================================================================
// Block size for kernel launch
// ============================================================================

#define AGG9_BLOCK_SIZE 256

// ============================================================================
// Relation ID constants (from SIMD reference)
// ============================================================================

#define AGG9_MEM_ID_TO_BIG_RELATION_ID  1662111297u
#define AGG9_RC_8_RELATION_ID           1420243005u
#define AGG9_PEM_RELATION_ID            2038149019u
#define AGG9_SELF_RELATION_ID           194336987u

// ============================================================================
// Initial accumulator constants -- the Starknet shift point as 28 M31 limbs
// ============================================================================

__constant__ uint32_t AGG9_SHIFT_POINT_X_LIMBS[28] = {
    427, 381, 378, 484, 320, 88, 319, 396,
    429, 59, 52, 478, 148, 181, 18, 309,
    412, 88, 2, 184, 126, 46, 29, 206,
    134, 233, 448, 211
};

__constant__ uint32_t AGG9_SHIFT_POINT_Y_LIMBS[28] = {
    508, 420, 374, 283, 306, 450, 278, 86,
    131, 160, 411, 301, 264, 322, 161, 346,
    320, 342, 261, 184, 280, 326, 220, 167,
    21, 260, 19, 199
};

// ============================================================================
// Felt252 type and field operations (prefixed with agg9_ to avoid linker
// conflicts with wb18 kernel which has identical static device functions)
// ============================================================================

typedef ff_storage<8> Felt252Field;

static __device__ __forceinline__ Felt252Field agg9_felt_add(
    const Felt252Field& a, const Felt252Field& b) {
    return ff_dispatch_st<ff_config_starknet>::add(a, b);
}

static __device__ __forceinline__ Felt252Field agg9_felt_sub(
    const Felt252Field& a, const Felt252Field& b) {
    return ff_dispatch_st<ff_config_starknet>::sub(a, b);
}

static __device__ __forceinline__ Felt252Field agg9_felt_to_mont(const Felt252Field& a) {
    return ff_dispatch_st<ff_config_starknet>::to_montgomery(a);
}

static __device__ __forceinline__ Felt252Field agg9_felt_from_mont(const Felt252Field& a) {
    return ff_dispatch_st<ff_config_starknet>::from_montgomery(a);
}

static __device__ __forceinline__ Felt252Field agg9_felt_mul(
    const Felt252Field& a, const Felt252Field& b) {
    return ff_dispatch_st<ff_config_starknet>::mul(a, b);
}

static __device__ __forceinline__ Felt252Field agg9_felt_inverse(const Felt252Field& a) {
...
static __device__ Felt252Field agg9_limbs28_to_felt252(const m31* limbs) {
    uint64_t accum = 0;
    int bit_pos = 0;
...
static __device__ void agg9_felt252_to_limbs28(const Felt252Field& felt, m31* limbs) {
...
static __device__ void agg9_ec_point_add(
    ...
    Felt252Field acc_x = agg9_limbs28_to_felt252(acc_x_limbs);
    Felt252Field acc_y = agg9_limbs28_to_felt252(acc_y_limbs);
    Felt252Field table_x = agg9_limbs28_to_felt252(table_x_limbs);
    Felt252Field table_y = agg9_limbs28_to_felt252(table_y_limbs);

    // slope = (table_y - acc_y) / (table_x - acc_x)
    Felt252Field dy = agg9_felt_sub(table_y, acc_y);
    Felt252Field dx = agg9_felt_sub(table_x, acc_x);
    Felt252Field num_mont = agg9_felt_to_mont(dy);
    Felt252Field denom_mont = agg9_felt_to_mont(dx);
    Felt252Field slope_mont = agg9_felt_mul(num_mont, agg9_felt_inverse(denom_mont));
    Felt252Field slope = agg9_felt_from_mont(slope_mont);

    // result_x = slope^2 - acc_x - table_x
    Felt252Field slope_sq = agg9_felt_from_mont(
        agg9_felt_mul(agg9_felt_to_mont(slope), agg9_felt_to_mont(slope)));
    Felt252Field rx = agg9_felt_sub(agg9_felt_sub(slope_sq, acc_x), table_x);

    // result_y = slope * (acc_x - result_x) - acc_y
    Felt252Field ry = agg9_felt_sub(
        agg9_felt_from_mont(
            agg9_felt_mul(agg9_felt_to_mont(slope),
                         agg9_felt_to_mont(agg9_felt_sub(acc_x, rx)))),
        acc_y);

    agg9_felt252_to_limbs28(rx, result_x_limbs);
    agg9_felt252_to_limbs28(ry, result_y_limbs);
}
...
struct agg9_kernel_args {
    ...
    m31** sub_pem;                  // 86 arrays, each 56*trace_size (sub_pem[col][round*trace_size + row])
};

__constant__ agg9_kernel_args d_agg9_args;

// ============================================================================
// Main trace generation kernel
// ============================================================================

__global__ void __launch_bounds__(AGG9_BLOCK_SIZE, 2)
agg9_trace_kernel() {
    uint32_t row = blockIdx.x * blockDim.x + threadIdx.x;
    if (row >= d_agg9_args.trace_size) return;

    m31** traces = d_agg9_args.traces;
    m31** inputs = d_agg9_args.inputs;
    ...
    memory_id_to_big_state_deduce_output(
        d_agg9_args.transpose_big_value_ptr,
        d_agg9_args.small_value_ptr,
    ...
    //    For window_bits_9: each limb IS a 9-bit window (no combining)
    // ==================================================================
    uint32_t m_shifted_a[28];
    for (int i = 0; i < 28; i++) {
        m_shifted_a[i] = (uint32_t)value_a_limbs[i];
    }

    uint32_t m_shifted_b[28];
    for (int i = 0; i < 28; i++) {
        m_shifted_b[i] = (uint32_t)value_b_limbs[i];
    }

    // ==================================================================
    // 7. Chain 0: 28 rounds of EC point addition using value A
    //    Initialize accumulator from shift point
    // ==================================================================
    m31 acc_x[28], acc_y[28];
    for (int i = 0; i < 28; i++) {
        acc_x[i] = (m31)AGG9_SHIFT_POINT_X_LIMBS[i];
        acc_y[i] = (m31)AGG9_SHIFT_POINT_Y_LIMBS[i];
    }
    ...
    for (int r = 0; r < 28; r++) {
        // Table lookup: 512 entries per round for 9-bit window
        uint32_t table_idx = 512u * (uint32_t)r + m_shifted_a[r];
        m31 table_x[28], table_y[28];
        for (int i = 0; i < 28; i++) {
            table_x[i] = g_pedersen_table_small_columns[i][table_idx];
        }
        for (int i = 0; i < 28; i++) {
            table_y[i] = g_pedersen_table_small_columns[28 + i][table_idx];
        }

        // Write PEM sub-component inputs for this round
        // sub_pem[col][round * trace_size + row] -- 86 cols, 56 rounds interleaved
        uint32_t pem_off = r * d_agg9_args.trace_size + row;  // chain 0, round r
        d_agg9_args.sub_pem[0][pem_off] = (m31)chain_id_0;
        d_agg9_args.sub_pem[1][pem_off] = (m31)((uint32_t)r);
        // Shifted m_shifted values: m_shifted_a[r..27], then zeros (28 values)
        for (int i = 0; i < 28; i++) {
            if (r + i < 28) {
                d_agg9_args.sub_pem[2 + i][pem_off] = (m31)m_shifted_a[r + i];
            } else {
                d_agg9_args.sub_pem[2 + i][pem_off] = 0u;
            }
        }
        // acc_x BEFORE addition (28 values)
        for (int i = 0; i < 28; i++) {
            d_agg9_args.sub_pem[30 + i][pem_off] = acc_x[i];
        }
        // acc_y BEFORE addition (28 values)
        for (int i = 0; i < 28; i++) {
            d_agg9_args.sub_pem[58 + i][pem_off] = acc_y[i];
        }

        // EC point addition: acc = acc + table_point
        m31 new_x[28], new_y[28];
        agg9_ec_point_add(acc_x, acc_y, table_x, table_y, new_x, new_y);
    ...
    // Write chain 0 trace cols 65-148: 28 zeros + 28 result_x + 28 result_y
    for (int i = 0; i < 28; i++) {
        traces[65 + i][row] = 0u;
    }
    for (int i = 0; i < 28; i++) {
        traces[93 + i][row] = chain0_result_x[i];
    }
    for (int i = 0; i < 28; i++) {
        traces[121 + i][row] = chain0_result_y[i];
    }

    // ==================================================================
    // 8. Chain 1: 28 rounds of EC point addition using value B
    ...
    for (int r = 0; r < 28; r++) {
        // Table lookup: 512 entries per round for 9-bit window, offset by 28 rounds
        uint32_t table_idx = 512u * (uint32_t)(28 + r) + m_shifted_b[r];
        m31 table_x[28], table_y[28];
        for (int i = 0; i < 28; i++) {
            table_x[i] = g_pedersen_table_small_columns[i][table_idx];
        }
        ...
        // Write PEM sub-component inputs for this round
        // sub_pem[col][(28 + r) * trace_size + row] -- chain 1, round r
        uint32_t pem_off = (28 + r) * d_agg9_args.trace_size + row;
        d_agg9_args.sub_pem[0][pem_off] = (m31)chain_id_1;
        d_agg9_args.sub_pem[1][pem_off] = (m31)((uint32_t)(28 + r));
        // Shifted m_shifted values: m_shifted_b[r..27], then zeros (28 values)
        for (int i = 0; i < 28; i++) {
            if (r + i < 28) {
                d_agg9_args.sub_pem[2 + i][pem_off] = (m31)m_shifted_b[r + i];
            } else {
                d_agg9_args.sub_pem[2 + i][pem_off] = 0u;
            }
        }
    ...
    // Write chain 1 trace cols 149-232: 28 zeros + 28 result_x + 28 result_y
    for (int i = 0; i < 28; i++) {
        traces[149 + i][row] = 0u;
    }
    for (int i = 0; i < 28; i++) {
        traces[177 + i][row] = chain1_result_x[i];
    }
    for (int i = 0; i < 28; i++) {
        traces[205 + i][row] = chain1_result_y[i];
    }

    // ==================================================================
    // 9. Multiplicity (col 233)
    // ==================================================================
    traces[233][row] = d_agg9_args.mults[row];
    ...
    d_agg9_args.lk_mem_0[0][row] = (m31)AGG9_MEM_ID_TO_BIG_RELATION_ID;
    d_agg9_args.lk_mem_0[1][row] = input_limb_0;
    for (int i = 0; i < 28; i++) {
        d_agg9_args.lk_mem_0[2 + i][row] = value_a_limbs[i];
    }

    // --- lk_mem_1: [relation_id, input_limb_1, value_b_limbs[0..27]] ---
    d_agg9_args.lk_mem_1[0][row] = (m31)AGG9_MEM_ID_TO_BIG_RELATION_ID;
    d_agg9_args.lk_mem_1[1][row] = input_limb_1;
    for (int i = 0; i < 28; i++) {
        d_agg9_args.lk_mem_1[2 + i][row] = value_b_limbs[i];
    }

    // --- lk_mem_2: [relation_id, input_limb_2, chain1_result_x[0..27]] ---
    d_agg9_args.lk_mem_2[0][row] = (m31)AGG9_MEM_ID_TO_BIG_RELATION_ID;
    d_agg9_args.lk_mem_2[1][row] = input_limb_2;
    for (int i = 0; i < 28; i++) {
        d_agg9_args.lk_mem_2[2 + i][row] = chain1_result_x[i];
    }

    // --- lk_rc8_0: [relation_id, value_a_limbs[27] - ms_is_max_a] ---
    d_agg9_args.lk_rc8_0[0][row] = (m31)AGG9_RC_8_RELATION_ID;
    d_agg9_args.lk_rc8_0[1][row] = (m31)((uint32_t)value_a_limbs[27] - ms_is_max_a);

    // --- lk_rc8_1: [relation_id, rc_input_a] ---
    d_agg9_args.lk_rc8_1[0][row] = (m31)AGG9_RC_8_RELATION_ID;
    d_agg9_args.lk_rc8_1[1][row] = (m31)rc_input_a;

    // --- lk_rc8_2: [relation_id, value_b_limbs[27] - ms_is_max_b] ---
    d_agg9_args.lk_rc8_2[0][row] = (m31)AGG9_RC_8_RELATION_ID;
    d_agg9_args.lk_rc8_2[1][row] = (m31)((uint32_t)value_b_limbs[27] - ms_is_max_b);

    // --- lk_rc8_3: [relation_id, rc_input_b] ---
    d_agg9_args.lk_rc8_3[0][row] = (m31)AGG9_RC_8_RELATION_ID;
    d_agg9_args.lk_rc8_3[1][row] = (m31)rc_input_b;

    // --- lk_pem_0: chain 0 input ---
    // [relation_id, chain_id_0, 0, m_shifted_a[0..27],
    //  shift_x[0..27], shift_y[0..27]]
    d_agg9_args.lk_pem_0[0][row] = (m31)AGG9_PEM_RELATION_ID;
    d_agg9_args.lk_pem_0[1][row] = (m31)chain_id_0;
    d_agg9_args.lk_pem_0[2][row] = 0u;
    for (int i = 0; i < 28; i++) {
        d_agg9_args.lk_pem_0[3 + i][row] = (m31)m_shifted_a[i];
    }
    for (int i = 0; i < 28; i++) {
        d_agg9_args.lk_pem_0[31 + i][row] = (m31)AGG9_SHIFT_POINT_X_LIMBS[i];
    }
    for (int i = 0; i < 28; i++) {
        d_agg9_args.lk_pem_0[59 + i][row] = (m31)AGG9_SHIFT_POINT_Y_LIMBS[i];
    }

    // --- lk_pem_1: chain 0 output ---
    // [relation_id, chain_id_0, 28, trace_cols[65..148]]
    d_agg9_args.lk_pem_1[0][row] = (m31)AGG9_PEM_RELATION_ID;
    d_agg9_args.lk_pem_1[1][row] = (m31)chain_id_0;
    d_agg9_args.lk_pem_1[2][row] = 28u;
    for (int i = 0; i < 84; i++) {
        d_agg9_args.lk_pem_1[3 + i][row] = traces[65 + i][row];
    }

    // --- lk_pem_2: chain 1 input ---
    // [relation_id, chain_id_1, 28, m_shifted_b[0..27],
    //  chain0_result_x[0..27], chain0_result_y[0..27]]
    d_agg9_args.lk_pem_2[0][row] = (m31)AGG9_PEM_RELATION_ID;
    d_agg9_args.lk_pem_2[1][row] = (m31)chain_id_1;
    d_agg9_args.lk_pem_2[2][row] = 28u;
    for (int i = 0; i < 28; i++) {
        d_agg9_args.lk_pem_2[3 + i][row] = (m31)m_shifted_b[i];
    }
    for (int i = 0; i < 28; i++) {
        d_agg9_args.lk_pem_2[31 + i][row] = chain0_result_x[i];
    }
    for (int i = 0; i < 28; i++) {
        d_agg9_args.lk_pem_2[59 + i][row] = chain0_result_y[i];
    }

    // --- lk_pem_3: chain 1 output ---
    // [relation_id, chain_id_1, 56, trace_cols[149..232]]
    d_agg9_args.lk_pem_3[0][row] = (m31)AGG9_PEM_RELATION_ID;
    d_agg9_args.lk_pem_3[1][row] = (m31)chain_id_1;
    d_agg9_args.lk_pem_3[2][row] = 56u;
    for (int i = 0; i < 84; i++) {
        d_agg9_args.lk_pem_3[3 + i][row] = traces[149 + i][row];
    }

    // --- lk_agg_0: self-lookup [relation_id, input_0, input_1, input_2] ---
    d_agg9_args.lk_agg_0[0][row] = (m31)AGG9_SELF_RELATION_ID;
    d_agg9_args.lk_agg_0[1][row] = input_limb_0;
    d_agg9_args.lk_agg_0[2][row] = input_limb_1;
    d_agg9_args.lk_agg_0[3][row] = input_limb_2;
    ...
    d_agg9_args.sub_mem[0][row] = input_limb_0;
    d_agg9_args.sub_mem[1][row] = input_limb_1;
    d_agg9_args.sub_mem[2][row] = input_limb_2;

    // range_check_8 feeds
    d_agg9_args.sub_rc8[0][row] = (m31)((uint32_t)value_a_limbs[27] - ms_is_max_a);
    d_agg9_args.sub_rc8[1][row] = (m31)rc_input_a;
    d_agg9_args.sub_rc8[2][row] = (m31)((uint32_t)value_b_limbs[27] - ms_is_max_b);
    d_agg9_args.sub_rc8[3][row] = (m31)rc_input_b;
...
extern "C" void gen_pedersen_aggregator_wb9_trace(
    ...
    // Each thread performs 56 EC point additions (28 per chain), each requiring
    ...
    m31** d_traces = clone_to_device<m31*>(traces, 234);
    ...
    m31** d_lk_pem_0 = clone_to_device<m31*>(lk_pem_0, 87);
    m31** d_lk_pem_1 = clone_to_device<m31*>(lk_pem_1, 87);
    m31** d_lk_pem_2 = clone_to_device<m31*>(lk_pem_2, 87);
    m31** d_lk_pem_3 = clone_to_device<m31*>(lk_pem_3, 87);
    ...
    m31** d_sub_pem = clone_to_device<m31*>(sub_pem, 86);

    // Fill kernel args struct and copy to constant memory
    agg9_kernel_args args;
    ...
    cudaMemcpyToSymbol(d_agg9_args, &args, sizeof(agg9_kernel_args));
...
// ============================================================================
// Interaction trace generation for pedersen_aggregator_wb9 (6 logup columns)
// ============================================================================
//
...
//   Col 3: partial_ec_mul pair (pem_0 - pem_1)  -- SUB: 87-elem combine each
//   Col 4: partial_ec_mul pair (pem_2 - pem_3)  -- SUB: 87-elem combine each
//   Col 5: mem_id_to_big_2 + self-lookup with mults -- SPECIAL
//
// All lookup data arrays reside on GPU (produced by gen_pedersen_aggregator_wb9_trace).
...
#define AGG9_IT_BLOCK_SIZE 256

// ADD pair kernel: frac = (d0 + d1) / (d0 * d1)
template <int N, int M>
__launch_bounds__(AGG9_IT_BLOCK_SIZE, 2)
__global__ void agg9_it_add_pair_kernel(
...
template <int N, int M>
__launch_bounds__(AGG9_IT_BLOCK_SIZE, 2)
__global__ void agg9_it_sub_pair_kernel(
    LookupElementsBasic<128>* lookup_elements,
    m31** data_0,
    m31** data_1,
    unsigned trace_size,
...
template <int N, int M>
__launch_bounds__(AGG9_IT_BLOCK_SIZE, 2)
__global__ void agg9_it_special_mults_kernel(
    LookupElementsBasic<128>* lookup_elements,
    m31** data_0,
    m31** data_1,
    m31* mults,
...
__global__ void agg9_it_finalize_col_kernel(
    unsigned rep_index,
    unsigned trace_size,
    qm31* denom_inv_ptr,
    m31* numerator0,
...
__global__ void agg9_it_cumsum_shift(
    unsigned n_cols,
    unsigned trace_size,
...
__global__ void agg9_it_coord_prefix_sum(
    m31* coordinate_sums,
    unsigned n_cols,
    unsigned trace_size,
    m31** interactive_traces
...
#define AGG9_IT_PROCESS_ADD(col_idx, N1, N2, d0_ptrs, d1_ptrs) \
...
extern "C" void gen_pedersen_aggregator_wb9_interaction_trace(
    ...
    uint32_t log_size,
    // Output
    m31** interaction_trace_columns,   // 4 * AGG9_N_LOGUP_COLUMNS = 24 columns
    ...
    m31** d_pem_0 = clone_to_device<m31*>(lk_pem_0, 87);
    m31** d_pem_1 = clone_to_device<m31*>(lk_pem_1, 87);
    m31** d_pem_2 = clone_to_device<m31*>(lk_pem_2, 87);
    m31** d_pem_3 = clone_to_device<m31*>(lk_pem_3, 87);
    ...
    m31** device_it = clone_to_device<m31*>(interaction_trace_columns, 4 * AGG9_N_LOGUP_COLUMNS);

    int block_dim = trace_size < AGG9_IT_BLOCK_SIZE ? trace_size : AGG9_IT_BLOCK_SIZE;
    int num_blocks = (trace_size + block_dim - 1) / block_dim;

    // Col 0: mem_id_to_big pair (mem_0 + mem_1) -- ADD, 30 elements each
    AGG9_IT_PROCESS_ADD(0, 30, 30, d_mem_0, d_mem_1);

    // Col 1: range_check_8 pair (rc8_0 + rc8_1) -- ADD, 2 elements each
    AGG9_IT_PROCESS_ADD(1, 2, 2, d_rc8_0, d_rc8_1);

    // Col 2: range_check_8 pair (rc8_2 + rc8_3) -- ADD, 2 elements each
    AGG9_IT_PROCESS_ADD(2, 2, 2, d_rc8_2, d_rc8_3);

    // Col 3: partial_ec_mul pair (pem_0 - pem_1) -- SUB, 87 elements each
    AGG9_IT_PROCESS_SUB(3, 87, 87, d_pem_0, d_pem_1);

    // Col 4: partial_ec_mul pair (pem_2 - pem_3) -- SUB, 87 elements each
    AGG9_IT_PROCESS_SUB(4, 87, 87, d_pem_2, d_pem_3);
    ...
    inclusive_prefix_sum(interaction_trace_columns[4 * AGG9_N_LOGUP_COLUMNS - 3], trace_size);
    inclusive_prefix_sum(interaction_trace_columns[4 * AGG9_N_LOGUP_COLUMNS - 2], trace_size);
    inclusive_prefix_sum(interaction_trace_columns[4 * AGG9_N_LOGUP_COLUMNS - 1], trace_size);
