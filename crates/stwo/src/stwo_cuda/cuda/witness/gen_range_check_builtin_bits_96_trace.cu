...
#include "gen_range_check_builtin_bits_96_trace.cuh"
#include "gen_memory_address_to_id_trace.cuh"
#include "gen_memory_id_to_big_trace.cuh"

// Base trace generation kernel for range_check_builtin_bits_96
// 12 trace columns, 1 memory_address_to_id lookup, 1 memory_id_to_big lookup, 1 range_check_6 lookup
__launch_bounds__(RANGE_CHECK_96_BUILTIN_TRACE_GEN_THREAD_COUNT_MAX, 2)
__global__ void generate_range_check_builtin_bits_96_trace_kernel(
    m31 **traces,
    ...
    if (row < trace_size) {
        ...
        // Read Positive Known Id Num Bits 96: memory_id_to_big
        // 96 bits = 10 × 9-bit limbs + 1 × 6-bit limb = 11 limbs total
        ...
        // Write the 11 limbs to trace columns 1-11
        m31 value_limb[11];
        for (int i = 0; i < 11; i++) {
        ...
        // The last limb (value_limb[10]) should be in range [0, 2^6)
        m31 value_limb_10_col11 = value_limb[10];

        // Fill sub-component inputs and lookup data for range_check_6
        sub_component_inputs_range_check_6[0][row] = value_limb_10_col11;
        lookup_range_check_6_0[0][row] = value_limb_10_col11;
        ...
        // Remaining 17 elements are zeros (96 bits uses only 11 limbs out of 28)
        for (int i = 11; i < 28; i++) {
...
void generate_range_check_builtin_bits_96_traces(
    ...
    timer global_timer;
    global_timer.start("generate range_check_builtin_bits_96 base trace");

    unsigned trace_size = 1 << log_size;

    dim3 blockDim(RANGE_CHECK_96_BUILTIN_TRACE_GEN_THREAD_COUNT_MAX);
    dim3 gridDim((trace_size + blockDim.x - 1) / blockDim.x);

    // Copy pointer arrays to device memory
    m31 **device_traces = clone_to_device<m31*>((m31**)traces, RANGE_CHECK_96_BUILTIN_N_TRACE_COLUMNS);
    ...
    // Range check 6 lookup (1 element)
    m31 **device_lookup_rc6_0 = clone_to_device<m31*>((m31**)lookup_range_check_6_0, 1);
    ...
    cuda_free_memory(device_lookup_rc6_0);
...
// Column generation kernel for first pair (memory_address_to_id + range_check_6)
__launch_bounds__(RANGE_CHECK_96_BUILTIN_TRACE_GEN_THREAD_COUNT_MAX, 2)
__global__ void generate_range_check_96_interaction_col0_kernel(
    MemoryAddressToId *memory_address_to_id,
    RangeCheck_6 *range_check_6,
    m31 **lookup_addr2id,
    m31 **lookup_rc6,
    unsigned trace_size,
    qm31 *denom_ptr,
    ...
    if (vec_index < trace_size) {
        ...
        m31 rc6_values[1] = {
            lookup_rc6[0][vec_index]
        };

        qm31 denom0 = memory_address_to_id->combine(addr2id_values, 2);
        qm31 denom1 = range_check_6->combine(rc6_values, 1);
        logup_col_write_frac(vec_index, add(denom0, denom1), mul(denom0, denom1),
                            denom_ptr, numerator0, numerator1, numerator2, numerator3);
    }
}

// Column generation kernel for second pair (memory_id_to_big single)
__launch_bounds__(RANGE_CHECK_96_BUILTIN_TRACE_GEN_THREAD_COUNT_MAX, 2)
__global__ void generate_range_check_96_interaction_col1_kernel(
    ...
    if (vec_index < trace_size) {
        ...
        qm31 denom = memory_id_to_big->combine(id2big_values, 29);
        // Single lookup: numerator = 1, denominator = denom
...
__global__ void generate_range_check_96_interaction_finalize_col_kernel(
    unsigned col_index,
    unsigned trace_size,
    qm31 *denom_inv_ptr,
    m31 *numerator0,
...
__global__ void generate_range_check_96_interaction_cumsum_shift_kernel(
    unsigned last_index,
    unsigned trace_size,
    m31 **interaction_traces,
    m31 *coordinate_sums
...
__global__ void generate_range_check_96_interaction_coord_prefix_sum_kernel(
    m31 *coordinate_sums,
    unsigned last_index,
    unsigned trace_size,
    m31 **interaction_traces
...
void generate_range_check_builtin_bits_96_interaction_traces(
    ...
    timer global_timer;
    global_timer.start("generate range_check_builtin_bits_96 interaction trace");

    unsigned trace_size = 1 << log_size;
    unsigned block_dim_val = RANGE_CHECK_96_BUILTIN_TRACE_GEN_THREAD_COUNT_MAX;
    ...
    RangeCheck_6 *device_rc6 = cuda_malloc<RangeCheck_6>(1);
    ASSERT_CUDA_SUCCESS(cudaMemcpyAsync(device_rc6, range_check_6, sizeof(RangeCheck_6), cudaMemcpyHostToDevice, 0));
    ...
    // Interaction trace (2 pairs × 4 m31 columns = 8 m31 columns total)
    m31 **device_interaction_trace = clone_to_device<m31*>((m31**)interaction_trace, RANGE_CHECK_96_BUILTIN_N_INTERACTION_TRACE_COLUMNS * 4);
    ...
    auto launch_finalize = [&](unsigned col_index) {
        ...
        generate_range_check_96_interaction_finalize_col_kernel<<<num_blocks, block_dim_val>>>(
    ...
    // Column 0: pair(memory_address_to_id_0, range_check_6_0)
    generate_range_check_96_interaction_col0_kernel<<<num_blocks, block_dim_val>>>(
        device_mem_addr_to_id, device_rc6,
        device_lookup_addr2id_0, device_lookup_rc6_0,
    ...
    // Column 1: single(memory_id_to_big_0)
    generate_range_check_96_interaction_col1_kernel<<<num_blocks, block_dim_val>>>(
    ...
    unsigned last_index = RANGE_CHECK_96_BUILTIN_N_INTERACTION_TRACE_COLUMNS;
    ...
    inclusive_prefix_sum(interaction_trace[4 * RANGE_CHECK_96_BUILTIN_N_INTERACTION_TRACE_COLUMNS - 4], trace_size);
    inclusive_prefix_sum(interaction_trace[4 * RANGE_CHECK_96_BUILTIN_N_INTERACTION_TRACE_COLUMNS - 3], trace_size);
    inclusive_prefix_sum(interaction_trace[4 * RANGE_CHECK_96_BUILTIN_N_INTERACTION_TRACE_COLUMNS - 2], trace_size);
    inclusive_prefix_sum(interaction_trace[4 * RANGE_CHECK_96_BUILTIN_N_INTERACTION_TRACE_COLUMNS - 1], trace_size);
    ...
    global_timer.end("generate range_check_builtin_bits_96 interaction trace");
}
