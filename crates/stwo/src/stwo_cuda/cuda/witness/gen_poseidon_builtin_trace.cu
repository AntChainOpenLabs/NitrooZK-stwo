...
__global__ void generate_poseidon_builtin_base_trace_kernel(
    ...
    if (row < trace_size) {
        ...
        for (int round = 0; round < 4; round++) {
            poseidon_full_round(state, round);
        }
        ...
        m31 chain_id = {row * 2};  // chain_id = seq * 2
        ...
        // poseidon_full_round_chain_1: [chain_id, round=4, cols 120-129, cols 130-139, cols 140-149]
        lookup_poseidon_full_round_chain_1[0][row] = chain_id;  // same chain_id
        lookup_poseidon_full_round_chain_1[1][row] = M31_4_val;  // round = 4
        for (int i = 0; i < 10; i++) {
            lookup_poseidon_full_round_chain_1[2 + i][row] = full_round_out_0[i];       // cols 120-129
            lookup_poseidon_full_round_chain_1[12 + i][row] = full_round_out_1[i];      // cols 130-139
            lookup_poseidon_full_round_chain_1[22 + i][row] = full_round_out_2[i];      // cols 140-149
        }
        ...
        for (int round = 31; round < 35; round++) {
            poseidon_full_round(state, round);
        }
...
__global__ void gen_poseidon_interaction_col3_kernel(
    ...
    // poseidon_full_round_chain_1: [chain_id, round=4, cols 120-149]
    m31 values_1[32];
    for (int i = 0; i < 32; i++) {
        values_1[i] = lookup_pfrc_1[i][row];
    }
...
extern "C" void gen_poseidon_builtin_interaction_trace(
    ...
    gen_poseidon_interaction_col3_kernel<<<num_blocks, BLOCK_SIZE>>>(
        d_poseidon_chain_rel, d_lookup_pfrc_0, d_lookup_pfrc_1,
        n_rows, d_denom, d_numerator0, d_numerator1, d_numerator2, d_numerator3
    );
    batch_inverse_secure_field(d_denom, d_denom_inv, n_rows);
    gen_poseidon_interaction_finalize_col_kernel<<<num_blocks, BLOCK_SIZE>>>(
