...
#include "gen_jump_opcode_rel_imm_trace.cuh"
#include "gen_memory_address_to_id_trace.cuh"
#include "gen_memory_id_to_big_trace.cuh"

__launch_bounds__(256, 2)
__global__ void generate_jump_opcode_rel_imm_trace_kernel(
    m31 **traces,
    ...
    m31 **jump_opcode_rel_imm_inputs,
    ...
    if (row < trace_size) {
        // Input columns
        m31 input_pc_col0 = jump_opcode_rel_imm_inputs[0][row];
        traces[0][row] = input_pc_col0;
        m31 input_ap_col1 = jump_opcode_rel_imm_inputs[1][row];
        traces[1][row] = input_ap_col1;
        m31 input_fp_col2 = jump_opcode_rel_imm_inputs[2][row];
        traces[2][row] = input_fp_col2;

        // Decode Instruction - get ap_update_add_1 from instruction at PC
        ...
        // ap_update_add_1 = (((limb5 >> 3) + (limb6 << 6)) >> 11) & 1
        uint16_t ap_update_add_1_tmp =
            ((((((uint16_t)(memory_id_to_big_value_tmp_1[5])) >> UInt16_3)
                ...
                >> UInt16_11)
                & UInt16_1);
        m31 ap_update_add_1_col3 = m31{ap_update_add_1_tmp};
        traces[3][row] = ap_update_add_1_col3;

        // verify_instruction sub_component_inputs and lookup
        // [pc, 32767, 32767, 32769, 56, (4 + (ap_update_add_1 * 32)), 0]
        m31 flags1 = add(M31_4, mul(ap_update_add_1_col3, M31_32));
        ...
        sub_component_inputss_verify_instruction[4][row] = M31_56;
        ...
        lookup_verify_instruction_0[4][row] = M31_56;
        lookup_verify_instruction_0[5][row] = flags1;
        lookup_verify_instruction_0[6][row] = M31_0;

        // Read Small - read the immediate value from PC+1
        ...
        m31 next_pc_id_col4 = next_pc_id_value;
        traces[4][row] = next_pc_id_col4;

        sub_component_inputss_memory_address_to_id[0][row] = pc_plus_1;
        lookup_memory_address_to_id_0[0][row] = pc_plus_1;
        lookup_memory_address_to_id_0[1][row] = next_pc_id_col4;
        ...
        // msb = (limb27 == 256)
        m31 msb_col5 = ((unsigned)next_pc_value[27] == 256u) ? M31_1 : M31_0;
        traces[5][row] = msb_col5;

        // mid_limbs_set = (limb20 == 511)
        m31 mid_limbs_set_col6 = ((unsigned)next_pc_value[20] == 511u) ? M31_1 : M31_0;
        traces[6][row] = mid_limbs_set_col6;

        // Write limbs 0-2 (columns 7-9)
        m31 next_pc_limb_0_col7 = next_pc_value[0];
        traces[7][row] = next_pc_limb_0_col7;
        m31 next_pc_limb_1_col8 = next_pc_value[1];
        traces[8][row] = next_pc_limb_1_col8;
        m31 next_pc_limb_2_col9 = next_pc_value[2];
        traces[9][row] = next_pc_limb_2_col9;

        // remainder_bits = limb3 & 3
        uint16_t remainder_bits_tmp = ((uint16_t)(unsigned)next_pc_value[3]) & UInt16_3;
        m31 remainder_bits_col10 = m31{remainder_bits_tmp};
        traces[10][row] = remainder_bits_col10;

        // partial_limb_msb = (remainder_bits & 2) >> 1
        uint16_t partial_limb_msb_tmp = (((uint16_t)remainder_bits_col10) & UInt16_2) >> UInt16_1;
        m31 partial_limb_msb_col11 = m31{partial_limb_msb_tmp};
        traces[11][row] = partial_limb_msb_col11;

        sub_component_inputss_memory_id_to_big[0][row] = next_pc_id_col4;

        // memory_id_to_big_0 lookup data with complex formula
        lookup_memory_id_to_big_0[0][row] = next_pc_id_col4;
        lookup_memory_id_to_big_0[1][row] = next_pc_limb_0_col7;
        lookup_memory_id_to_big_0[2][row] = next_pc_limb_1_col8;
        lookup_memory_id_to_big_0[3][row] = next_pc_limb_2_col9;
        // ((remainder_bits) + ((mid_limbs_set) * (508)))
        lookup_memory_id_to_big_0[4][row] = add(remainder_bits_col10, mul(mid_limbs_set_col6, M31_508));
        // ((mid_limbs_set) * (511)) for limbs 5-20
        m31 mid_limbs_fill = mul(mid_limbs_set_col6, M31_511);
        for (int i = 5; i <= 20; i++) {
            lookup_memory_id_to_big_0[i][row] = mid_limbs_fill;
        }
        // (((136) * (msb)) - (mid_limbs_set))
        lookup_memory_id_to_big_0[21][row] = sub(mul(M31_136, msb_col5), mid_limbs_set_col6);
        // zeros for limbs 22-27
        for (int i = 22; i <= 27; i++) {
            lookup_memory_id_to_big_0[i][row] = M31_0;
        }
        // ((msb) * (256))
        lookup_memory_id_to_big_0[28][row] = mul(msb_col5, M31_256);
        ...
        m31 next_pc = sub(sub(
            add(add(add(
                next_pc_limb_0_col7,
                mul(next_pc_limb_1_col8, M31_512)),
                mul(next_pc_limb_2_col9, M31_262144)),
                mul(remainder_bits_col10, M31_134217728)),
            msb_col5),
            mul(M31_536870912, mid_limbs_set_col6));

        // next_pc = pc + read_small_output
        lookup_opcodes_1[0][row] = add(input_pc_col0, next_pc);
        lookup_opcodes_1[1][row] = add(input_ap_col1, ap_update_add_1_col3);
        lookup_opcodes_1[2][row] = input_fp_col2;

        // Enabler column (col 12)
        traces[12][row] = (row < n_rows) ? M31_1 : M31_0;
    }
}

void generate_jump_opcode_rel_imm_traces(
    unsigned **traces,
    ...
    unsigned **jump_opcode_rel_imm_input,
    ...
    m31 **device_traces = clone_to_device<m31*>((m31 **)traces, JUMP_OPCODE_REL_IMM_N_TRACE_COLUMNS);
    ...
    m31 **device_jump_opcode_rel_imm_input = clone_to_device<m31 *>((m31 **)jump_opcode_rel_imm_input, 3);
    unsigned **device_memory_id_to_big_transposed_big_values = clone_to_device<m31 *>((m31 **)memory_id_to_big_transposed_big_values, 8);


    timer global_timer;
    global_timer.start("generate jump_opcode_rel_imm base trace");

    unsigned trace_size = 1 << log_size;
    int block_dim = trace_size < JUMP_OPCODE_REL_IMM_TRACE_GEN_THREAD_COUNT_MAX ? trace_size : JUMP_OPCODE_REL_IMM_TRACE_GEN_THREAD_COUNT_MAX;
    int num_blocks = block_dim < JUMP_OPCODE_REL_IMM_TRACE_GEN_THREAD_COUNT_MAX ? 1 : (trace_size + block_dim - 1) / block_dim;

    generate_jump_opcode_rel_imm_trace_kernel<<<num_blocks, block_dim>>>(
        device_traces,
        ...
        device_jump_opcode_rel_imm_input,
        memory_address_to_id_address_to_raw_id,
    ...
    global_timer.end("generate jump_opcode_rel_imm base trace");
    ...
    cuda_free_memory(device_jump_opcode_rel_imm_input);
...
__launch_bounds__(256, 2)
__global__ void generate_jump_opcode_rel_imm_interaction_trace_col_gen_kernel_round0(
...
template <int N, int M>
__launch_bounds__(256, 2)
__global__ void generate_jump_opcode_rel_imm_interaction_trace_col_gen_kernel_second2last(
    LookupElementsBasic<N> *lookup_elements_n,
    LookupElementsBasic<M> *lookup_elements_m,
    m31 **lookup_state_0,
    m31 **lookup_state_1,
...
template <int N>
__launch_bounds__(256, 2)
__global__ void generate_jump_opcode_rel_imm_interaction_trace_col_gen_kernel_last(
    LookupElementsBasic<N> *lookup_elements_n,
    m31 **lookup_state_0,
    unsigned trace_size,
    qm31 *denom_ptr,
...
__global__ void generate_jump_opcode_rel_imm_interaction_trace_finalize_col_kernel(
    unsigned rep_index,
    unsigned trace_size,
    qm31 *denom_inv_ptr,
    m31 *numerator0,
...
__global__ void generate_jump_opcode_rel_imm_interaction_trace_cumsum_shift(
    unsigned last_index,
    unsigned trace_size,
    m31 **interaction_traces,
    m31 *coordinate_sums
...
__global__ void generate_jump_opcode_rel_imm_interaction_trace_coord_prefix_sum(
    m31 *coordinate_sums,
    unsigned last_index,
    unsigned trace_size,
    m31 **interaction_traces
...
void generate_jump_opcode_rel_imm_interaction_traces(
    ...
    m31 **device_interaction_traces = clone_to_device<m31*>((m31 **)interaction_traces, 4 * JUMP_OPCODE_REL_IMM_N_INTERACTION_TRACE_COLUMNS);
    ...
    timer global_timer;
    global_timer.start("generate jump_opcode_rel_imm interaction trace");

    int block_dim = 0;
    int num_blocks = 0;

    block_dim = trace_size < JUMP_OPCODE_REL_IMM_TRACE_GEN_THREAD_COUNT_MAX ? trace_size : JUMP_OPCODE_REL_IMM_TRACE_GEN_THREAD_COUNT_MAX;
    num_blocks = block_dim < JUMP_OPCODE_REL_IMM_TRACE_GEN_THREAD_COUNT_MAX ? 1 : (trace_size + block_dim - 1) / block_dim;

    // #0 Interaction trace For verify_instruction_0 & memory_address_to_id_0
    generate_jump_opcode_rel_imm_interaction_trace_col_gen_kernel_round0<7, 2><<<num_blocks, block_dim>>>(
    ...
    generate_jump_opcode_rel_imm_interaction_trace_finalize_col_kernel<<<num_blocks, block_dim>>>(
        0,
        trace_size,
        denom_inv,
        device_numerator0,
    ...
    block_dim = trace_size < JUMP_OPCODE_REL_IMM_TRACE_GEN_THREAD_COUNT_MAX ? trace_size : JUMP_OPCODE_REL_IMM_TRACE_GEN_THREAD_COUNT_MAX;
    num_blocks = block_dim < JUMP_OPCODE_REL_IMM_TRACE_GEN_THREAD_COUNT_MAX ? 1 : (trace_size + block_dim - 1) / block_dim;
    generate_jump_opcode_rel_imm_interaction_trace_col_gen_kernel_second2last<29, 3><<<num_blocks, block_dim>>>(
    ...
    block_dim = trace_size < JUMP_OPCODE_REL_IMM_TRACE_GEN_THREAD_COUNT_MAX ? trace_size : JUMP_OPCODE_REL_IMM_TRACE_GEN_THREAD_COUNT_MAX;
    num_blocks = block_dim < JUMP_OPCODE_REL_IMM_TRACE_GEN_THREAD_COUNT_MAX ? 1 : (trace_size + block_dim - 1) / block_dim;
    generate_jump_opcode_rel_imm_interaction_trace_col_gen_kernel_last<3><<<num_blocks, block_dim>>>(
        device_opcodes_lookup_elements,
        device_lookup_opcodes_1,
        trace_size,
        device_logup_denom,
    ...
    generate_jump_opcode_rel_imm_interaction_trace_cumsum_shift<<<num_blocks, block_dim, shared_size>>>(
        JUMP_OPCODE_REL_IMM_N_INTERACTION_TRACE_COLUMNS,
        trace_size,
        device_interaction_traces,
        (m31 *)claimed_sum
    );
    ...
    block_dim = trace_size < THREAD_COUNT_MAX ? trace_size : THREAD_COUNT_MAX;
    num_blocks = block_dim < THREAD_COUNT_MAX ? 1 : (trace_size + block_dim - 1) / block_dim;
    generate_jump_opcode_rel_imm_interaction_trace_coord_prefix_sum<<<num_blocks, block_dim>>>(
        (m31 *)claimed_sum,
        JUMP_OPCODE_REL_IMM_N_INTERACTION_TRACE_COLUMNS,
        trace_size,
        device_interaction_traces
    ...
    inclusive_prefix_sum((m31 *)interaction_traces[4 * JUMP_OPCODE_REL_IMM_N_INTERACTION_TRACE_COLUMNS - 4], trace_size);
    inclusive_prefix_sum((m31 *)interaction_traces[4 * JUMP_OPCODE_REL_IMM_N_INTERACTION_TRACE_COLUMNS - 3], trace_size);
    inclusive_prefix_sum((m31 *)interaction_traces[4 * JUMP_OPCODE_REL_IMM_N_INTERACTION_TRACE_COLUMNS - 2], trace_size);
    inclusive_prefix_sum((m31 *)interaction_traces[4 * JUMP_OPCODE_REL_IMM_N_INTERACTION_TRACE_COLUMNS - 1], trace_size);

    global_timer.end("generate jump_opcode_rel_imm interaction trace");
