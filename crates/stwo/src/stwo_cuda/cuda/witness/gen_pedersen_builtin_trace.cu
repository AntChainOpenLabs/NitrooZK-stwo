/**
 * CUDA trace generation for pedersen_builtin (3-col trace).
 *
 * Simplified kernel — only 3 output columns. All EC math is delegated to
 * the pedersen_aggregator component.
 *
 * Architecture:
 *   - 1 thread per row
...
#include "gen_pedersen_builtin_trace.cuh"
#include "../fields.cuh"
#include "../utils.cuh"
#include "../logup.cuh"
...
#define PED_BLOCK_SIZE 256
#define PED_MEM_ADDR_TO_ID_RELATION_ID 1444891767u
#define PED_AGG_RELATION_ID            520578465u
#define PED_N_LOGUP_COLUMNS            2

// ============================================================================
// Base trace kernel
// ============================================================================

__global__ void gen_pedersen_builtin_trace_kernel(
    ...
    m31 rel_id = m31(PED_MEM_ADDR_TO_ID_RELATION_ID);
    ...
    // Lookup data: pedersen_aggregator (relation_id = 520578465)
    lk_agg0_0[idx] = m31(PED_AGG_RELATION_ID);
    lk_agg0_1[idx] = id0;
    lk_agg0_2[idx] = id1;
    lk_agg0_3[idx] = id2;
...
extern "C" void gen_pedersen_builtin_trace(
    ...
    int num_blocks = (trace_size + PED_BLOCK_SIZE - 1) / PED_BLOCK_SIZE;

    gen_pedersen_builtin_trace_kernel<<<num_blocks, PED_BLOCK_SIZE>>>(
...
#define PED_IT_BLOCK_SIZE 256

// ADD pair kernel: frac = (d0 + d1) / (d0 * d1)
template <int N, int M>
__launch_bounds__(PED_IT_BLOCK_SIZE, 2)
__global__ void ped_it_add_pair_kernel(
...
__global__ void ped_it_finalize_col_kernel(
    unsigned rep_index,
    unsigned trace_size,
    qm31* denom_inv_ptr,
    m31* numerator0,
...
__global__ void ped_it_cumsum_shift(
    unsigned n_cols,
    unsigned trace_size,
...
__global__ void ped_it_coord_prefix_sum(
    m31* coordinate_sums,
    unsigned n_cols,
    unsigned trace_size,
    m31** interactive_traces
...
#define PED_IT_PROCESS_ADD(col_idx, N1, N2, d0_ptrs, d1_ptrs) \
...
extern "C" void gen_pedersen_builtin_interaction_trace(
    ...
    m31** lk_agg_0,          // 4 arrays
    uint32_t log_size,
    m31** interaction_trace_columns,   // 4 * PED_N_LOGUP_COLUMNS = 8 columns
    ...
    m31** device_it = clone_to_device<m31*>(interaction_trace_columns, 4 * PED_N_LOGUP_COLUMNS);

    int block_dim = trace_size < PED_IT_BLOCK_SIZE ? trace_size : PED_IT_BLOCK_SIZE;
    int num_blocks = (trace_size + block_dim - 1) / block_dim;

    // Col 0: memory_address_to_id pair (mem_0 + mem_1) — ADD, 3 elements each
    PED_IT_PROCESS_ADD(0, 3, 3, d_mem_0, d_mem_1);

    // Col 1: memory_address_to_id_2 + pedersen_aggregator_0 — ADD, 3/4 elements
    PED_IT_PROCESS_ADD(1, 3, 4, d_mem_2, d_agg_0);
    ...
    inclusive_prefix_sum(interaction_trace_columns[4 * PED_N_LOGUP_COLUMNS - 3], trace_size);
    inclusive_prefix_sum(interaction_trace_columns[4 * PED_N_LOGUP_COLUMNS - 2], trace_size);
    inclusive_prefix_sum(interaction_trace_columns[4 * PED_N_LOGUP_COLUMNS - 1], trace_size);
