...
// Host-side pointers for table management
extern m31** h_pedersen_table_device_ptrs;
extern bool g_pedersen_table_initialized;
...
// Device-side table pointers (allocated in constant memory for fast access)
// Note: For very large tables, we use regular global memory
__device__ m31* g_pedersen_table_columns[PEDERSEN_TABLE_N_COLUMNS];
__device__ uint32_t g_pedersen_table_n_rows = 0;

// Kernel to copy table column pointers to device global variables
__global__ void set_pedersen_table_pointers_kernel(m31** column_ptrs, uint32_t n_rows) {
...
// Declarations only - definitions are in gen_pedersen_builtin_trace.cu with extern "C" linkage
// void pedersen_table_init(m31** columns, uint32_t n_rows);
// void pedersen_table_free();

// Internal implementation functions (called by extern "C" wrappers)
