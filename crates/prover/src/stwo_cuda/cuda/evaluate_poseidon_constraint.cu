...
__global__ void evaluate_poseidon_constraint_pre_kernel(
    ...
    for (unsigned instance_index = 0; instance_index < N_INSTANCES_PER_ROW; instance_index++) {
        ...
        for (unsigned i = 0; i < N_STATE; i++) {
            state[i] = next_trace_mask(trace1_evaluations, col_index, row, domain_log_size, eval_domain_log_size);
        ...
        for (unsigned round = 0; round < N_HALF_FULL_ROUNDS; round++) {
            ...
            for (unsigned i = 0; i < N_STATE; i++) {
                const m31 m = next_trace_mask(trace1_evaluations, col_index, row, domain_log_size, eval_domain_log_size);
                add_constraint(random_coeff_powers, &constraint_index, sub(state[i], m), &row_res);
                state[i] = m;
            }
        }
        ...
        for (unsigned round = 0; round < N_PARTIAL_ROUNDS; round++) {
            ...
            {
                const m31 m = next_trace_mask(trace1_evaluations, col_index, row, domain_log_size, eval_domain_log_size);
                add_constraint(random_coeff_powers, &constraint_index, sub(state[0], m), &row_res);
                state[0] = m;
            }
        }
        ...
        Fraction f1 = add_to_relation<N_STATE>(entry);
        intermediate_fractions[row * N_INSTANCES_PER_ROW * 2 + 2 * instance_index] = f1;

        RelationEntry entry2 = RelationEntry<N_STATE>(poseidon_eval->lookup_elements, qm31{{2147483646,0}, {0,0}}, state);
        Fraction f2 = add_to_relation<N_STATE>(entry2);
        intermediate_fractions[row * N_INSTANCES_PER_ROW * 2 + 2 * instance_index + 1] = f2;
    }

    numerators[row] = row_res;
    constraint_index_array[row] = constraint_index;
}

__launch_bounds__(256, 2)
__global__ void process_constraint_post_kernel(
    qm31 *numerators,
    Fraction *intermediate_fractions,
    unsigned *constraint_index_array,
    ...
    Fraction fractions[N_INSTANCES_PER_ROW * 2];
    for (int i = 0; i < N_INSTANCES_PER_ROW * 2; ++i) {
        fractions[i] = intermediate_fractions[row * N_INSTANCES_PER_ROW * 2 + i];
    }

    unsigned col_index[3] = {0};
    unsigned constraint_index = constraint_index_array[row];
    qm31 row_res = numerators[row];
    const unsigned logup_interaction = 2;

    unsigned batching[N_INSTANCES_PER_ROW * 2];
    for (int i = 0; i < N_INSTANCES_PER_ROW * 2; ++i) {
        batching[i] = i / 2;
    }
    const unsigned last_batch = batching[N_INSTANCES_PER_ROW * 2 - 1];
    qm31 prev_col_cumsum = { {0, 0}, {0, 0} };

    for (unsigned i = 0; i < last_batch; ++i) {
        const Fraction cur_frac = Fraction::sum(&fractions[2 * i], 2);
        qm31 cur_cumsum_arr[1] = { { {0, 0}, {0, 0} } };
        int offsets[1] = { 0 };
        next_extension_interaction_mask(trace2_evaluations, col_index, logup_interaction, offsets, 2, row, domain_log_size, eval_domain_log_size, cur_cumsum_arr);
        ...
        add_constraint_ext(random_coeff_powers, &constraint_index, constrain_val, &row_res);
    }
    {
        const Fraction frac_sum = Fraction::sum(&fractions[last_batch * 2], 2);
        int offsets2[2] = { 0, -1 };
        qm31 cumsum2[2] = { { {0, 0}, {0, 0} }, { {0, 0}, {0, 0} } };
        next_extension_interaction_mask(trace2_evaluations, col_index, logup_interaction, offsets2, 2, row, domain_log_size, eval_domain_log_size, cumsum2);
...
__global__ void evaluate_poseidon_constraint_quotients_finalize_kernel(
...
void evaluate_poseidon_constraint_quotients_on_domain(
    ...
    Fraction *d_intermediate_fractions = cuda_malloc<Fraction>(eval_domain_size * N_INSTANCES_PER_ROW * 2);
    ...
    evaluate_poseidon_constraint_pre_kernel<<<num_blocks, block_dim>>>(
        numerators,
    ...
    process_constraint_post_kernel<<<num_blocks, block_dim>>>(
        numerators,
        d_intermediate_fractions,
        constrain_index_array,
    ...
    evaluate_poseidon_constraint_quotients_finalize_kernel<<<num_blocks, block_dim>>>(
        quotients_0,
        quotients_1,
        quotients_2,
        quotients_3,
